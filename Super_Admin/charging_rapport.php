  ...
  <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
    <thead>
      <tr>
		...
		<th style="background-color: #f2f2f2; font-weight: bold; padding: 10px; text-align: left; border-bottom: 1px solid #ddd;">Charging date</th>
        <th style="background-color: #f2f2f2; font-weight: bold; padding: 10px; text-align: left; border-bottom: 1px solid #ddd;">Charging Quantity</th>
      </tr>
    </thead>
    <tbody>
...
if (!empty($filterDate) && !empty($filterAgent)) {
    ...
    $query = "SELECT username,charge_date,id_cas,id_case,emp_case,NI,codice,qte_charg FROM charging_report WHERE username LIKE '%$filterAgent%'";
} else {
...
if ($result->num_rows > 0) {
  ...
  echo "<td style='padding: 10px; text-align: left; border-bottom: 1px solid #ddd; background-color: #F9F9F9;'>" . $row['charge_date'] . "</td>";
  echo "<td style='padding: 10px; text-align: left; border-bottom: 1px solid #ddd; background-color: #F9F9F9;'>" . $row['qte_charg'] . "</td>";
  echo "</tr>";
...
?>
  ...
  <form method="GET" action="download_charg.php">
