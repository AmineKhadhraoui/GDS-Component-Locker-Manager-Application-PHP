...
if (!empty($filterDate) && !empty($filterAgent)) {
    ...
    $query = "SELECT username,dump_date,id_cas,id_case,emp_case,NI,codice,qte_dump FROM uncharging_repport WHERE DATE(dump_date) = '$filterDate' AND username LIKE '%$filterAgent%'";
} elseif (!empty($filterDate)) {
    // Only date filter is provided
    $query = "SELECT username,retour_date,id_cas,id_case,emp_case,NI,codice,qte_retour FROM uncharging_repport WHERE DATE(dump_date) = '$filterDate'";
} elseif (!empty($filterAgent)) {
    // Only agent filter is provided
    $query = "SELECT username,dump_date,id_cas,id_case,emp_case,NI,codice,qte_dump FROM uncharging_repport WHERE username LIKE '%$filterAgent%'";} else {
