  ...
  <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
    <thead>
      <tr>
        ...
        <th style="background-color: #f2f2f2; font-weight: bold; padding: 10px; text-align: left; border-bottom: 1px solid #ddd;">Returning Quantity</th>
        <th style="background-color: #f2f2f2; font-weight: bold; padding: 10px; text-align: left; border-bottom: 1px solid #ddd;">Returning date</th>
      </tr>
    </thead>
    <tbody>
...
if (!empty($filterDate) && !empty($filterAgent)) {
    ...
    $query = "SELECT username,id_cas,id_case,emp_case,NI,codice,qte_retour,retour_date FROM retour WHERE DATE(retour_date) = '$filterDate' AND username LIKE '%$filterAgent%'";
} elseif (!empty($filterDate)) {
    // Only date filter is provided
    $query = "SELECT username,id_cas,id_case,emp_case,NI,codice,qte_retour,retour_date FROM retour WHERE DATE(retour_date) = '$filterDate' ";
} elseif (!empty($filterAgent)) {
    // Only agent filter is provided
    $query = "SELECT username,id_cas,id_case,emp_case,NI,codice,qte_retour,retour_date FROM retour WHERE  username LIKE '%$filterAgent%'";
} else {
...
if ($result->num_rows > 0) {
  ...
  echo "<td style='padding: 10px; text-align: left; border-bottom: 1px solid #ddd; background-color: #F9F9F9;'>" . $row['qte_retour'] . "</td>";
  echo "<td style='padding: 10px; text-align: left; border-bottom: 1px solid #ddd; background-color: #F9F9F9;'>" . $row['retour_date'] . "</td>";
...
?>
  ...
  <form method="GET" action="download_return.php">
